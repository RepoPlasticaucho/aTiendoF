...
@Component({
  selector: 'app-portafolios-comprar-dev',
  templateUrl: './portafolios-comprar-dev.component.html',
  styleUrls: ['./portafolios-comprar-dev.component.css']
})
export class PortafoliosComprarDevComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.httpService.obtenermodeloproducto$.subscribe((res) => {
      if (res.id == '') {
        ...
        this.router.navigate(['/navegation-cl', { outlets: { 'contentClient': ['portafolios-dev'] } }]);
      } else {
  ...
  verCarrito() {
    this.router.navigate(['/navegation-cl', { outlets: { 'contentClient': ['carrito-dev'] } }]);
  }
}
